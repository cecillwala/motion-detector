...
class FaceEyeDetector:
    ...
    def _extract_eye_points(self, landmarks, eye_indices, frame_width, frame_height):
        """
        Extract 6 eye landmark coordinates from MediaPipe results.

        MediaPipe returns landmarks as NORMALIZED coordinates (0.0 to 1.0).
        We need to convert them to actual pixel coordinates.

        Args:
            landmarks: MediaPipe face landmarks object
                       (results.multi_face_landmarks[0].landmark)
            eye_indices: List of 6 indices (LEFT_EYE_INDICES or RIGHT_EYE_INDICES)
            frame_width: Width of the frame in pixels
            frame_height: Height of the frame in pixels

        Returns:
            List of 6 (x, y) tuples in pixel coordinates
        ...
        What to do:
            ...
            for idx in eye_indices:
                ...
                # lm.x is the horizontal position (0=left, 1=right)
                # lm.y is the vertical position (0=top, 1=bottom)
        ...
        Example:
            If lm.x = 0.5 and frame_width = 640:
                x = int(0.5 * 640) = 320 (middle of frame)
            If lm.y = 0.3 and frame_height = 480:
                y = int(0.3 * 480) = 144 (upper third of frame)
        """
        # YOUR CODE HERE
        points = []
        for idx in eye_indices:
    ...
    def detect(self, frame):
        """
        Run face and eye detection on a single frame.
        ...
        What to do:
            ...
            #2. Convert BGR to RGB (MediaPipe requires RGB):
               rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

               WHY: OpenCV captures frames in BGR (Blue-Green-Red)
               order. MediaPipe expects RGB (Red-Green-Blue). Without
               this conversion, the model gets confused and landmarks
               will be wrong or faces won't be detected.

            #3. Process the frame:
               results = self.face_mesh.process(rgb)

               This runs the full Face Mesh neural network on the frame.
               It returns an object where results.multi_face_landmarks
               is either:
                 - A list of detected faces (each with 468 landmarks)
            ...
            #4. Check if a face was found:
               ...
               IMPORTANT: Check for None/falsy, not empty list.
               MediaPipe returns None when no face is detected.

            #5. Get landmarks for the first face:
               face_landmarks = results.multi_face_landmarks[0].landmark

               We only process the first face (max_num_faces=1).

            #6. Extract eye points:
